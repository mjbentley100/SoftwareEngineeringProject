...
class TrainingDataGenerator:
	...
	def importData(self, el, mfccs, chroma_stft, spec_cent, spec_bw, rolloff, zcr):
		db_connection = self.connectDB()
...
def main():
	...
	arr = TDG.getClassifierArray()
	#print(arr)
	for el in arr:
		if (el != "-"):
			mffcs, chroma_stft, spec_cent, spec_bw, rolloff, zcr = TDG.performMFCC("../girl_sing_sample/%s-%s/sample_%s.mp3" % (folder_no, folder_no + 49, str(count).zfill(3)))
			try:
				TDG.importData(el, mffcs, chroma_stft, spec_cent, spec_bw, rolloff, zcr)
			except:
				continue
		count = count + 1
		if (count % 50 == 0):
